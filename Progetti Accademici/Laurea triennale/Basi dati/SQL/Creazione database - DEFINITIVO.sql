...
unique(ID_Gate, data_Inizio),	-- perchè un gate non può essere assegnato a più compagnie aeree nella stessa data
check (data_Inizio < data_Fine),
primary key(ID),
...
unique(ID_Strumento, data_Inizio),	-- perchè uno strumento non può assegnare nella stessa data a più gate
check (data_Inizio < data_Fine),
primary key(ID),
foreign key (ID_Gate) references Gate(ID),
